...
def  main():
  ...
  installer = "\installer\obsvstlink_vst Installer.exe"
   
  if demo:
    installer = "\installer\obsvstlink_vst Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\obsvstlink_vst manual.pdf" 
  ]
  ...
  ZIPNAME = "obsvstlink_vst-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "obsvstlink_vst-v" + FULLVERSIONSTR + "-win-demo.zip"
